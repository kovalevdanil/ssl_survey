...
strbuf_t *scan_domain(char *domain)
{
    ...
    SSL_CTX *contexts[PROTO_COUNT] = {0};
    int available_proto[PROTO_COUNT] = {0};

    // creating context for every protocol versions
    ...
    // checking all protocol versions
    for (int i = 0; i < PROTO_COUNT; i++)
    {
    ...
    // search of min and max protocol versions
    ...
    // geting pubkey length
    X509 *cert;
    int bits = 0;

    bio = BIO_create_and_connect(contexts[max_ind], domain);
    BIO_get_ssl(bio, &ssl);
